...
func AddCommentInRedis(comment *model.Comment) error {
	...
	pipe.HSet(global.CONTEXT, keyComment, "content", comment.Content)
	pipe.HSet(global.CONTEXT, keyComment, "user_id", userIDStr)
	pipe.HSet(global.CONTEXT, keyComment, "video_id", videoIDStr)
	pipe.HSet(global.CONTEXT, keyComment, "created_at", time.Now().UnixMilli())
...
func GoComment(comment model.Comment) error {
	...
	lua := redis.NewScript(`
				...
				if redis.call("Exists", key) <= 0 then
					redis.call("HSet", key, "video_id", video_id)
					redis.call("HSet", key, "user_id", user_id)
					redis.call("HSet", key, "content", content)
					redis.call("HSet", key, "created_at", created_at)
